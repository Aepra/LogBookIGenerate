...
export async function uploadFileToActivityFolder(params: UploadFileParams): Promise<UploadFileResult | null> {
  ...
  trace.log("UPLOAD_VERIFY", "verifying uploaded file...");

  // VERIFY ORPHAN FIX: retry verification once before giving up
  let meta = await getDriveFileMeta(trace, currentToken, refreshToken, fileId);
  if (!meta) {
    trace.warn("UPLOAD_VERIFY", "first verify attempt failed, retrying once...", { fileId });
    await sleep(250); // brief delay before retry
    meta = await getDriveFileMeta(trace, currentToken, refreshToken, fileId);
  }

  if (!meta) {
    // Upload succeeded but verification failed — this file is an orphan.
    // Best-effort cleanup: delete the file from Drive.
    trace.error("UPLOAD_VERIFY", "verify failed after retry, cleaning up orphan", { fileId });
    try {
      await fetch(`${DRIVE_API_BASE}/files/${fileId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${currentToken}` },
      });
      trace.log("UPLOAD_VERIFY", "orphan file deleted from Drive", { fileId });
    } catch (cleanupErr) {
      trace.error("UPLOAD_VERIFY", "orphan cleanup failed", { error: String(cleanupErr), fileId });
    }
    return null;
  }

  trace.log("UPLOAD_VERIFY", "verified", { name: meta.name });
  ...
  trace.log("UPLOAD_DONE", "complete", { fileId, webViewLink: meta.webViewLink });
  return { fileId, webViewLink: meta.webViewLink };
}
