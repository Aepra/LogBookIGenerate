...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check for overlap if start_time and end_time are provided
    if (start_time && end_time) {
      const { getActivitiesByLogbookId, checkOverlap } = await import("@/services/activity.service");
      const existingActivities = await getActivitiesByLogbookId(logbook_id);
      const { hasOverlap, conflictingActivity } = checkOverlap(
        existingActivities,
        activity_date,
        start_time,
        end_time
      ...
      if (hasOverlap && conflictingActivity) {
        return NextResponse.json(
          {
            error: `Waktu bentrok dengan aktivitas "${conflictingActivity.title}" (${conflictingActivity.start_time} - ${conflictingActivity.end_time})`,
          },
          { status: 409 }
        );
      }
